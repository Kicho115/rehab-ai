...
class BodySkeleton: Entity {
    ...
    private func createBoneEntity(for skeletonBone: SkeletonBone, diameter: Float = 0.04, color: UIColor = .white, brightness: Float = 2.0) -> Entity {
        let mesh = MeshResource.generateBox(size: [diameter, diameter, skeletonBone.length], cornerRadius: diameter / 2)
        let adjustedColor = color.withAlphaComponent(CGFloat(brightness))
        var material = SimpleMaterial(color: adjustedColor, roughness: 0.5, isMetallic: true)
        let entity = ModelEntity(mesh: mesh, materials: [material])
        return entity
    }
}
